...
# Homework 6: A working Tetris game you will extend without breaking.
 ...
 # -- region
  # call to tetris class 
    # sets up the basic visuals - board - key bindings - buttons
    # calls to run the game  
      # this check gameOver? if not repeatedly call itself with board run
        # board run drops current piece by one 
        # (move method of piece makes sure no collisions)
 # -- endregion

require_relative './hw6graphics'

# -- region 
  # constructor gets called by class method next_piece inside of the board 
  # `@current_block = Piece.next_piece(self) where self is board`
  # point array is points randomly given by class variable all pieces 
...
class Piece 
  ...
  # -- region 
    # sample takes one random element of the array  
  # -- endregion
  ...
  # takes the intended movement in x, y and rotation and   and returns true. 
...
class Tetris
  ...
  # check if game is over if not run via board which moves current down 
  # and replaces it with a new one when the old one can't move 
  def run_game
